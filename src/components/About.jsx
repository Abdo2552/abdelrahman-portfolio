...
export default function About() {
  ...
  return (
    <section id="about" className="py-28 px-6">
      <div ref={ref} className="reveal max-w-6xl mx-auto grid md:grid-cols-[1fr_1.3fr] gap-14">
        <div>
          <span className="font-[var(--font-mono)] text-xs text-[var(--color-blue)]">
            01 — About
          </span>
          <h2 className="font-[var(--font-display)] font-semibold text-3xl sm:text-4xl mt-3">
            A student who builds things to understand them.
          </h2>
        </div>

        <div className="space-y-6 text-[var(--color-muted)] leading-relaxed text-base sm:text-lg">
          <p>
            I'm a Software Engineering student at{" "}
            <span className="text-[var(--color-text)]">{profile.university}</span>
          ...
          <div className="grid sm:grid-cols-3 gap-4 pt-4">
            <InfoCard icon={<GraduationCap size={18} />} label="Studying" value={profile.department} />
            <InfoCard icon={<Code2 size={18} />} label="Graduating" value={profile.gradYear} />
            <InfoCard icon={<MapPin size={18} />} label="Based in" value={profile.location} />
          </div>
        </div>
      </div>
    </section>
...
function InfoCard({ icon, label, value }) {
  return (
    <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
      <div className="text-[var(--color-blue)] mb-2">{icon}</div>
      <p className="font-[var(--font-mono)] text-[11px] text-[var(--color-muted)] uppercase tracking-wide">
        {label}
      </p>
      <p className="text-[var(--color-text)] font-medium mt-0.5">{value}</p>
    </div>
  );
}
